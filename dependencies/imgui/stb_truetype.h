...
static stbtt__active_edge* stbtt__new_active(stbtt__hheap* hh, stbtt__edge* e, int off_x, float start_point, void* userdata)
{
    ...
    z->fx -= off_x;
    z->m_direction = e->invert ? 1.0f : -1.0f;
    z->sy = e->y0;
    z->ey = e->y1;
    z->next = 0;
    return z;
...
static void stbtt__handle_clipped_edge(float* scanline, int x, stbtt__active_edge* e, float x0, float y0, float x1, float y1)
{
    ...
    if (x0 <= x && x1 <= x)
        scanline[x] += e->m_direction * (y1 - y0);
    else if (x0 >= x + 1 && x1 >= x + 1)
        ;
    else {
        STBTT_assert(x0 >= x && x0 <= x + 1 && x1 >= x && x1 <= x + 1);
        scanline[x] += e->m_direction * (y1 - y0) * (1 - ((x0 - x) + (x1 - x)) / 2); // coverage = 1 - average x position
    }
}
        ...
        if (e->fdx == 0) {
            ...
            if (x_top >= 0 && x_bottom >= 0 && x_top < len && x_bottom < len) {
                ...
                if ((int)x_top == (int)x_bottom) {
                    ...
                    scanline[x] += e->m_direction * (1 - ((x_top - x) + (x_bottom - x)) / 2) * height;
                    scanline_fill[x] += e->m_direction * height; // everything right of this pixel is filled
                }
                else {
                    int x, x1, x2;
